...
from utils import get_ellipse_iou, bbox_attr


class SumSquaredErrorLoss(nn.Module):
    def __init__(self, l_coord=5.0, l_theta=5.0, l_noobj=0.5):
        super().__init__()
    ...
    def forward(self, p, a):
        # Calculate ellipse IOU (with theta) for responsible box assignment
        iou = get_ellipse_iou(p, a)              # (batch, S, S, B, B)
        max_iou = torch.max(iou, dim=-1)[0]     # (batch, S, S, B)

        # Get masks
        conf_idx = config.BBOX_ATTRS - 1
        bbox_mask = bbox_attr(a, conf_idx) > 0.0
        p_template = bbox_attr(p, conf_idx) > 0.0
